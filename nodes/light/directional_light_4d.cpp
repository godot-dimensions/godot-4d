...
bool DirectionalLight4D::update_light_3d_cross_section_render_base(const Projection &p_relative_to_camera_basis, const Vector4 &p_relative_to_camera_position, const RID p_light_3d_render_base) const {
	...
	// Both the cross-section and projected 3D representations need the projected angular
	// extent of the 4D light source, which is calculated in the projected update function.
	...
	// Therefore, always call the projected update function, even for the fallback case.
	if (!update_light_3d_projected_render_base(p_relative_to_camera_basis, p_relative_to_camera_position, p_light_3d_render_base)) {
		return false;
	}
...
bool DirectionalLight4D::update_light_3d_projected_render_base(const Projection &p_relative_to_camera_basis, const Vector4 &p_relative_to_camera_position, const RID p_light_3d_render_base) const {
	ERR_FAIL_COND_V_MSG(!p_light_3d_render_base.is_valid(), false, "DirectionalLight4D render base RID for Light3D is invalid.");
	RenderingServer *rendering_server = RenderingServer::get_singleton();
	ERR_FAIL_NULL_V(rendering_server, false);
	const Basis4D relative_basis_4d = Basis4D(p_relative_to_camera_basis);
