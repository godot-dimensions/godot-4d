...
Rect4 Node4D::get_rect_bounds_local(const Transform4D &p_to_target) const {
	PackedVector4Array result;
	GDVIRTUAL_CALL(_get_rect_bounds_local, p_to_target.basis, p_to_target.origin, result);
	if (result.size() != 2) {
...
Rect4 Node4D::get_rect_bounds_global(const Transform4D &p_to_target) const {
	return get_rect_bounds_local(p_to_target * get_global_transform());
}

PackedVector4Array Node4D::get_rect_bounds_global_bind(const Projection &p_transform_to_target_basis, const Vector4 &p_transform_to_target_origin) const {
	Rect4 bounds = get_rect_bounds_global(Transform4D(p_transform_to_target_basis, p_transform_to_target_origin));
	return PackedVector4Array({ bounds.position, bounds.size });
}

Rect4 Node4D::get_rect_bounds_global_recursive(const Transform4D &p_to_target) const {
	Rect4 bounds = get_rect_bounds_global(p_to_target);
	const int child_count = get_child_count();
	for (int i = 0; i < child_count; i++) {
		Node4D *child_4d = Object::cast_to<Node4D>(get_child(i));
		if (child_4d != nullptr) {
			bounds = bounds.merge(child_4d->get_rect_bounds_global_recursive(p_to_target));
		}
	}
	return bounds;
}

PackedVector4Array Node4D::get_rect_bounds_global_recursive_bind(const Projection &p_transform_to_target_basis, const Vector4 &p_transform_to_target_origin) const {
	Rect4 bounds = get_rect_bounds_global_recursive(Transform4D(p_transform_to_target_basis, p_transform_to_target_origin));
	return PackedVector4Array({ bounds.position, bounds.size });
}
...
void Node4D::_bind_methods() {
	...
	// Rect bounds.
	ClassDB::bind_method(D_METHOD("get_rect_bounds_local", "to_target_basis", "to_target_origin"), &Node4D::get_rect_bounds_local_bind, DEFVAL(Projection()), DEFVAL(Vector4()));
	ClassDB::bind_method(D_METHOD("get_rect_bounds_global", "to_target_basis", "to_target_origin"), &Node4D::get_rect_bounds_global_bind, DEFVAL(Projection()), DEFVAL(Vector4()));
