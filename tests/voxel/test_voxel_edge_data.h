...
TEST_CASE("[PackedVoxelEdgeData] Encoding round trips") {
	...
	for (int axis_a = 0; axis_a < 4; axis_a++) {
		for (int axis_b = axis_a + 1; axis_b < 4; axis_b++) {
			for (int signs = 0; signs < 4; signs++) {
				...
				const VoxelEdgeData decoded = PackedVoxelEdgeData::encode(VoxelEdgeData(normal, 0.5f)).decode();
				CHECK_MESSAGE(Math::abs(decoded.normal.dot(normal.normalized())) > 0.9999f, "PackedVoxelEdgeData should represent the centers of the hypercube's 2-faces exactly.");
			}
		}
	}

	CHECK_MESSAGE(PackedVoxelEdgeData::encode(VoxelEdgeData(Vector4(0, 1, 0, 0), 0.5f)).decode().normal == Vector4(0, 1, 0, 0), "PackedVoxelEdgeData should store axis-aligned normals exactly.");
	CHECK_MESSAGE(PackedVoxelEdgeData::encode(VoxelEdgeData(Vector4(1, 0, 0, 0), 0.0f)).decode().position == 0.0f, "PackedVoxelEdgeData should store a crossing at the edge's start exactly.");
	CHECK_MESSAGE(PackedVoxelEdgeData::encode(VoxelEdgeData(Vector4(1, 0, 0, 0), 1.0f)).decode().position == 1.0f, "PackedVoxelEdgeData should store a crossing at the edge's end exactly.");
}
} // namespace TestVoxelEdgeData
