...
TEST_CASE("[TigerTestGenerator] Materials and edge data") {
	...
	CHECK_MESSAGE(generator->get_material(Vector4i(10, 0, 10, 0)) == VoxelMaterial::SOLID, "TigerTestGenerator should be solid on the tiger's core circles.");
	...
	REQUIRE(generator->get_material(Vector4i(13, 0, 10, 0)) == VoxelMaterial::SOLID);
	...
	CHECK_MESSAGE(normal.is_normalized(), "TigerTestGenerator normals should decode to unit vectors.");
	CHECK_MESSAGE(normal.x > 0.6, "TigerTestGenerator normals should point along the surface gradient.");

	// The tolerance covers the crossing's linear approximation and its
	// quantization to 5 bits.
	Vector4 crossing_point = Vector4(13.5, 0.5, 10.5, 0.5);
	crossing_point.x += edge_data.decode_position();
