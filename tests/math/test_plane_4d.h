...
TEST_CASE("[Plane4D] Intersect Ray") {
	...
	Variant result = plane.intersect_ray_point(ray_origin, ray_down);
	CHECK_MESSAGE(result.get_type() == Variant::NIL, "Plane4D intersect_ray_point should return NIL for invalid hits.");

	const Vector4 ray_up = Vector4(0, 1, 0, 0);
	result = plane.intersect_ray_point(ray_origin, ray_up);
	CHECK_MESSAGE(result.get_type() == Variant::VECTOR4, "Plane4D intersect_ray_point should return a Vector4 for valid hits.");
	CHECK_MESSAGE(result == Vector4(1, 5, 3, 4), "Plane4D intersect_ray_point should work as expected.");
}
