...
class G4MFAccessor4D : public G4MFItem4D {
	...
	String _component_type = ""; // No default, must be explicitly set.
	int _buffer_view_index = -1;
	int _vector_size = 1;

	// Private functions for determining the minimal component type.
	static constexpr uint32_t CANT_USE_COMP_TYPE = 1000000; // Any very big number will do.
	static bool _double_bits_equal(const double p_a, const double p_b);
	static void _minimal_component_bits_for_double_only(const double p_value, uint32_t &r_float_bits);
	static void _minimal_component_bits_for_double(const double p_value, uint32_t &r_float_bits, uint32_t &r_int_bits, uint32_t &r_uint_bits);
	static void _minimal_component_bits_for_int64(const int64_t p_value, uint32_t &r_int_bits, uint32_t &r_uint_bits);
	static String _minimal_component_type_given_bits(const uint32_t p_float_bits, const uint32_t p_int_bits, const uint32_t p_uint_bits);
...
public:
	...
	String get_component_type() const { return _component_type; }
	void set_component_type(const String &p_component_type) { _component_type = p_component_type; }
	...
	int64_t get_bytes_per_component() const;
	...
	static String minimal_component_type_for_vector3s(const PackedVector3Array &p_input_data);
	...
	PackedVector3Array decode_vector3s_from_bytes(const Ref<G4MFState4D> &p_g4mf_state) const;
	...
	static Ref<G4MFAccessor4D> make_new_accessor_without_data(const String &p_component_type, const int p_vector_size = 1);
	...
	static int encode_new_accessor_from_int32s(const Ref<G4MFState4D> &p_g4mf_state, const PackedInt32Array &p_input_data, const int p_vector_size = 1, const bool p_deduplicate = true);
	static int encode_new_accessor_from_int64s(const Ref<G4MFState4D> &p_g4mf_state, const PackedInt64Array &p_input_data, const int p_vector_size = 1, const bool p_deduplicate = true);
