...
Ref<ShaderMaterial> Material4D::get_cross_section_material_3d() {
	if (_cross_section_material_3d.is_null()) {
		...
		_cross_section_material_3d->set_name(material_path_or_name + String(" Cross-Section Material"));
		update_cross_section_material_3d();
	}
	return _cross_section_material_3d;
}

Ref<ShaderMaterial> Material4D::get_projected_material_3d() {
	if (_projected_material_3d.is_null()) {
		_projected_material_3d.instantiate();
		const String material_path_or_name = get_path().is_empty() ? get_name() : get_path();
		_projected_material_3d->set_name(material_path_or_name + String(" Projected Material"));
		update_projected_material_3d();
	}
	return _projected_material_3d;
}
...
void Material4D::_bind_methods() {
	...
	ADD_PROPERTY(PropertyInfo(Variant::VECTOR3, "texture_map_offset", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_STORAGE), "set_texture_map_offset", "get_texture_map_offset");
	ADD_PROPERTY(PropertyInfo(Variant::VECTOR3, "texture_map_scale", PROPERTY_HINT_LINK, "", PROPERTY_USAGE_STORAGE), "set_texture_map_scale", "get_texture_map_scale");
