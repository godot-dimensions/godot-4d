...
void TetraMesh4D::update_cross_section_mesh() {
	...
	for (int i = 0; i < cell_indices.size(); i += 4) {
		...
		// - ImmediateMesh to compute cross-section on the CPU, but that's likely slower.
		// - Some kind of compute shader or custom render pipeline, but that's not supported on the Compatability renderer.
		...
		// Not enough slots left for normals. Also interpolating the 4D normals gives weird results, needs more experimentaiton. 
		// Currently flat normals are computed in the vertex shader.

		//// Vertices:

		// Not storing actual position data in the vertex positions, x is an index, yz are part of the normals data
