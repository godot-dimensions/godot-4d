...
Dictionary CylinderShape4D::raycast_intersects(const Vector4 &p_local_from, const Vector4 &p_local_direction, const real_t p_max_distance, const bool p_inside_is_zero) const {
	Dictionary result;
	result["hit"] = false;
	...
	// Check if the ray is parallel to the cylinder axis (Y axis).
	if (radial_dir_len_sq <= CMP_EPSILON2) {
		...
		if (radial_point_len_sq <= radius_sq && Math::abs(p_local_direction.y) > CMP_EPSILON2) {
			...
			if (distance_to_cap >= 0.0f && distance_to_cap < best_distance) {
				...
				result["point"] = p_local_from + p_local_direction * distance_to_cap;
			}
		}
	...
	if (Math::abs(p_local_direction.y) > CMP_EPSILON2) {
		...
		if (distance_to_top_cap >= 0.0f && distance_to_top_cap < best_distance) {
