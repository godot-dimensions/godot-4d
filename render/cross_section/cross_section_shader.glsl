...
// Each triange in the cross-section is made up of vertices that are interpolated along the edges of the tetrahedron, 
...
int get_face_lookup_index(float w1, float w2, float w3, float w4, int vertex_id) {
    // Bitwise ops limit compatability so summing instead.
...
void vertex() {
    ...
    if (CROSS_SECTION_LOOKUP[face] == -1) {
        ...
        POSITION = vec4(0.0, 0.0, CLIP_SPACE_FAR+1.0, 1.0);
    } else {
