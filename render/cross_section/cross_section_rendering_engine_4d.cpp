...
void CrossSectionRenderingEngine4D::update_camera() {
	...
	if (camera->get_projection_type() == Camera4D::PROJECTION4D_ORTHOGRAPHIC) {
		RenderingServer::get_singleton()->camera_set_orthogonal(_cross_section_camera, camera->get_orthographic_size(), camera->get_clip_near(), camera->get_clip_far());
	} else if (bool(camera->get_projection_type() & Camera4D::PROJECTION4D_PERSPECTIVE_3D)) {
		RenderingServer::get_singleton()->camera_set_perspective(_cross_section_camera, Math::rad_to_deg(camera->get_field_of_view_3d()), camera->get_clip_near(), camera->get_clip_far());
