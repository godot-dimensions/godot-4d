...
vec3 apply_gradient_sun(vec3 sky, vec4 eye_direction_4d, bool enabled, vec4 light_direction_4d, vec3 light_color, float light_energy, float light_angular_radius_radians) {
	if (!enabled) {
		return sky;
	}
...
vec3 render_gradient_sky(vec4 eye_direction_4d) {
	...
	sky_color = apply_gradient_sun(sky_color, eye_direction_4d, light0_enabled, light0_direction_4d, light0_color.rgb, light0_energy, light0_angular_radius_radians);
	sky_color = apply_gradient_sun(sky_color, eye_direction_4d, light1_enabled, light1_direction_4d, light1_color.rgb, light1_energy, light1_angular_radius_radians);
	sky_color = apply_gradient_sun(sky_color, eye_direction_4d, light2_enabled, light2_direction_4d, light2_color.rgb, light2_energy, light2_angular_radius_radians);
	sky_color = apply_gradient_sun(sky_color, eye_direction_4d, light3_enabled, light3_direction_4d, light3_color.rgb, light3_energy, light3_angular_radius_radians);

	vec3 ground = mix(bottom_color.rgb, horizon_color.rgb, clamp(pow(1.0 + v_angle, inv_bottom_curve), 0.0, 1.0));
	return mix(ground, sky_color, step(0.0, v_angle)) * energy_multiplier;
}
