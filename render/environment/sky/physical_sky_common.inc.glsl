#include "sky_render_parameters_4d.inc.glsl"
...
float physical_sky_henyey_greenstein(float cos_theta, float g) {
	const float k = 0.0795774715459;
	return k * (1.0 - g * g) / pow(1.0 + g * g - 2.0 * g * cos_theta, 1.5);
}

vec4 evaluate_physical_sun(vec4 eye_direction_4d, bool enabled, vec4 light_direction_4d, vec3 light_color, float light_energy, float light_angular_radius_radians, float rayleigh_scatter, float mie_scatter, bool draw_sun_glow) {
	if (!enabled) {
		return vec4(0.0);
	}
	...
	float mie_phase = physical_sky_henyey_greenstein(cos_theta, mie_anisotropy);
...
vec3 render_physical_sky(vec4 eye_direction_4d, bool cubemap_pass) {
	// Optical length.
	...
	bool draw_sun_glow = !cubemap_pass;
	vec4 sun0 = evaluate_physical_sun(eye_direction_4d, light0_enabled, light0_direction_4d, light0_color.rgb, light0_energy, light0_angular_radius_radians, rayleigh_scatter, mie_scatter, draw_sun_glow);
	vec4 sun1 = evaluate_physical_sun(eye_direction_4d, light1_enabled, light1_direction_4d, light1_color.rgb, light1_energy, light1_angular_radius_radians, rayleigh_scatter, mie_scatter, draw_sun_glow);
	vec4 sun2 = evaluate_physical_sun(eye_direction_4d, light2_enabled, light2_direction_4d, light2_color.rgb, light2_energy, light2_angular_radius_radians, rayleigh_scatter, mie_scatter, draw_sun_glow);
	vec4 sun3 = evaluate_physical_sun(eye_direction_4d, light3_enabled, light3_direction_4d, light3_color.rgb, light3_energy, light3_angular_radius_radians, rayleigh_scatter, mie_scatter, draw_sun_glow);

	vec3 color = sun0.rgb + sun1.rgb + sun2.rgb + sun3.rgb;
	float sun_fade = max(max(sun0.a, sun1.a), max(sun2.a, sun3.a));
	return pow(max(color, vec3(0.0)), vec3(1.0 / (1.2 + 1.2 * sun_fade))) * energy_multiplier;
}
