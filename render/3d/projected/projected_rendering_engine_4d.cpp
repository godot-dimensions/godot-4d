...
void ProjectedRenderingEngine4D::_render_frame_callback() {
	...
	if (mesh_instance_object_ids.size() > instances_allocated) {
		_instances_3d.resize(mesh_instance_object_ids.size());
	}
	int64_t instance_index = 0;
	for (int mesh_index = 0; mesh_index < mesh_instance_object_ids.size(); mesh_index++) {
		const ObjectID mesh_instance_object_id = (ObjectID)mesh_instance_object_ids[mesh_index];
		MeshInstance4D *mesh_instance = Object::cast_to<MeshInstance4D>(ObjectDB::get_instance(mesh_instance_object_id));
		...
		Ref<Mesh> mesh_3d = mesh_4d->get_proxy_mesh_3d(); // TODO: rename this method. It works for both.
...
void ProjectedRenderingEngine4D::_normalize_image_callback(int64_t p_effect_callback_type, RenderData *p_render_data) {
	...
	if (buffers) {
		...
		push_constant.resize(16);
		{
			...
			push_constant_data[3] = _cross_section_depth_texture.get_type() == Variant::NIL ? 0 : 1;
		}
